	...
	)
{
	...
	/*if (Channels == 3)
	{
		cvtColor(image_noisy, image_noisy, CV_BGR2YUV);
	}*/

	vector<Mat> block_noisy;//store the patch
	vector<int>row_idx;//patch idx along the row direction
	vector<int>col_idx;
	getPatches(image_noisy, Width, Height, Channels, kHard, pHard,block_noisy,row_idx, col_idx);
	...
	for (int i = 0; i < bn_r; i++)
	{
		for (int j = 0; j < bn_c; j++)
		{
			...
			tran1d(data);//3-D transforming

			weight_hd=shrink(data,lambda3d*sigma,sigma);//shrink the cofficient

			//weight_hd = calculate_weight_hd(data,sigma);

			inv_tran_3d(data);//3-D inverse transforming
	...
	getPatches(image_basic, Width, Height, Channels, kHard, pHard, block_basic, row_idx, col_idx);
	bn_r = row_idx.size();
	bn_c = col_idx.size();
